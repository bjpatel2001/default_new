...
<head>
    ...
    <style>
        /*.dataTables_processing.panel.panel-default{background-color: #4c8bf5;color: #ffffff;font-size: 15px;font-weight: bold;}*/
        .dataTables_wrapper#dataTableLog_wrapper{padding: 15px;}
        .dataTables_wrapper#dataTableLog_wrapper div.row:nth-child(2){margin: 20px 0;}
        .deta-table{opacity: 0.1;}
        table.dataTable.nowrap td{white-space: inherit !important;}
    </style>
    ...
    <div class="flash-message">
        @foreach (['danger', 'warning', 'success', 'info'] as $msg)
            @if(Session::has('alert-' . $msg))
                <p class="alert alert-{{ $msg }}" style="text-align:center;text-transform:capitalize;">{{ Session::get('alert-' . $msg) }} <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a></p>
            @endif
        @endforeach
    </div>
    @yield('content')
...
<script src="{{url('js/html5lightbox/html5lightbox.js')}}" type="text/javascript"></script>
